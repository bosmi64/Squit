...
void Counter_LED_Init(void) 
{
            ...
            uint8 ctrl;
        #endif /* (!Counter_LED_UsingFixedFunction && !Counter_LED_ControlRegRemoved) */
        ...
        Counter_LED_SetInterruptMode(Counter_LED_INIT_INTERRUPTS_MASK);
            ...
            (void)Counter_LED_ReadStatusRegister();
...
void Counter_LED_Enable(void) 
{
    ...
    /* else make sure reset is clear */
        ...
        Counter_LED_CONTROL |= Counter_LED_CTRL_ENABLE;                
    #endif /* (!Counter_LED_ControlRegRemoved || Counter_LED_UsingFixedFunction) */
...
*
* Global variables:
*  Counter_LED_initVar: Is modified when this function is called for the  
...
void Counter_LED_Stop(void) 
{
    /* Disable Counter */
    #if(!Counter_LED_ControlRegRemoved || Counter_LED_UsingFixedFunction)
        Counter_LED_CONTROL &= ((uint8)(~Counter_LED_CTRL_ENABLE));        
    #endif /* (!Counter_LED_ControlRegRemoved || Counter_LED_UsingFixedFunction) */
    
    /* Globally disable the Fixed Function Block chosen */
    #if (Counter_LED_UsingFixedFunction)
        Counter_LED_GLOBAL_ENABLE &= ((uint8)(~Counter_LED_BLOCK_EN_MASK));
        Counter_LED_GLOBAL_STBY_ENABLE &= ((uint8)(~Counter_LED_BLOCK_STBY_EN_MASK));
    #endif /* (Counter_LED_UsingFixedFunction) */
}
                                   ...
                                   
{
        ...
        CYASSERT (0u == (Counter_LED_GLOBAL_ENABLE & Counter_LED_BLOCK_EN_MASK));
...
uint8 Counter_LED_ReadCounter(void) 
{
    ...
    #if(Counter_LED_UsingFixedFunction)
		(void)CY_GET_REG16(Counter_LED_COUNTER_LSB_PTR);
	#else
		(void)CY_GET_REG8(Counter_LED_COUNTER_LSB_PTR_8BIT);
	#endif/* (Counter_LED_UsingFixedFunction) */
    
    /* Read the data from the FIFO (or capture register for Fixed Function)*/
    #if(Counter_LED_UsingFixedFunction)
        return ((uint8)CY_GET_REG16(Counter_LED_STATICCOUNT_LSB_PTR));
    #else
...
{
    #if(Counter_LED_UsingFixedFunction)
        return ((uint8)CY_GET_REG16(Counter_LED_PERIOD_LSB_PTR));
    #else
...
void Counter_LED_SetCompareMode(uint8 compareMode) 
{
    ...
    Counter_LED_CONTROL &= ((uint8)(~Counter_LED_CTRL_CMPMODE_MASK));
    
    /* Write the new setting */
    Counter_LED_CONTROL |= compareMode;
}
...
void Counter_LED_SetCaptureMode(uint8 captureMode) 
{
    ...
    Counter_LED_CONTROL &= ((uint8)(~Counter_LED_CTRL_CAPMODE_MASK));
    
    /* Write the new setting */
    Counter_LED_CONTROL |= ((uint8)((uint8)captureMode << Counter_LED_CTRL_CAPMODE0_SHIFT));
}
...
void Counter_LED_ClearFIFO(void) 
{
    ...
    while(0u != (Counter_LED_ReadStatusRegister() & Counter_LED_STATUS_FIFONEMP))
    {
        (void)Counter_LED_ReadCapture();
    }
