...
void Clock_SYS_Start(void) 
{
    ...
    Clock_SYS_CLKEN |= Clock_SYS_CLKEN_MASK;
	Clock_SYS_CLKSTBY |= Clock_SYS_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_SYS_Stop
********************************************************************************
*
* Summary:
...
void Clock_SYS_Stop(void) 
{
    ...
    Clock_SYS_CLKEN &= (uint8)(~Clock_SYS_CLKEN_MASK);
	Clock_SYS_CLKSTBY &= (uint8)(~Clock_SYS_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_SYS_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_SYS_CLKEN & Clock_SYS_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_SYS_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_SYS_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_SYS__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_SYS_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_SYS_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_SYS_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_SYS_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_SYS_GetSourceRegister();
    uint16 oldDivider = Clock_SYS_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_SYS_CLKEN & Clock_SYS_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_SYS_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_SYS_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_SYS_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_SYS_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_SYS_MOD_SRC |= modeBitMask & (uint8)Clock_SYS_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_SYS_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_SYS_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_SYS_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_SYS_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_SYS_MOD_SRC & (uint8)(Clock_SYS_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_SYS_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_SYS_GetDividerRegister();
    uint8 oldSrc = Clock_SYS_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_SYS_MOD_SRC |= CYCLK_SSS;
        Clock_SYS_MOD_SRC =
            (Clock_SYS_MOD_SRC & (uint8)(~Clock_SYS_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_SYS_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_SYS_MOD_SRC & Clock_SYS_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_SYS_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_SYS_PHASE = clkPhase & Clock_SYS_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_SYS_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_SYS_PHASE & Clock_SYS_PHASE_MASK;
}
