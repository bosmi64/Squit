...
void UART_USB_Init(void) 
{
        ...
        /* Write Counter Value for TX Bit Clk Generator*/
            ...
            UART_USB_TXBITCLKTX_COMPLETE_REG = (UART_USB_NUMBER_OF_DATA_BITS +
                        UART_USB_NUMBER_OF_START_BIT) * UART_USB_OVER_SAMPLE_COUNT;
        #else
    ...
    uint8 UART_USB_ReadRxData(void) 
    {
        uint8 rxData;
            ...
            /* Disable Rx interrupt. */
                ...
                UART_USB_DisableRxInt();
            #endif /* UART_USB_RX_INTERRUPT_ENABLED */
            loc_rxBufferRead = UART_USB_rxBufferRead;
            loc_rxBufferWrite = UART_USB_rxBufferWrite;

            if( (UART_USB_rxBufferLoopDetect != 0u) || (loc_rxBufferRead != loc_rxBufferWrite) )
            {
                rxData = UART_USB_rxBuffer[loc_rxBufferRead];
                loc_rxBufferRead++;

                if(loc_rxBufferRead >= UART_USB_RXBUFFERSIZE)
                {
                    loc_rxBufferRead = 0u;
                }
                /* Update the real pointer */
                UART_USB_rxBufferRead = loc_rxBufferRead;
    ...
    void UART_USB_PutChar(uint8 txDataByte) 
    {
                ...
                */
                ...
                do{
                    ...
                    #if ((UART_USB_TXBUFFERSIZE > UART_USB_MAX_BYTE_VALUE) && (CY_PSOC3))
                        /* Disable TX interrupt to protect variables that could change on interrupt */
                        CyIntDisable(UART_USB_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                    loc_txBufferWrite = UART_USB_txBufferWrite;
                    loc_txBufferRead = UART_USB_txBufferRead;
                    #if ((UART_USB_TXBUFFERSIZE > UART_USB_MAX_BYTE_VALUE) && (CY_PSOC3))
                ...
                {
                    if(loc_txBufferWrite >= UART_USB_TXBUFFERSIZE)
                    {
                        loc_txBufferWrite = 0u;
                    }
                    /* Add to the software buffer. */
                    UART_USB_txBuffer[loc_txBufferWrite] = txDataByte;
                    loc_txBufferWrite++;
                    ...
                    /* Finally, update the real output pointer */
                        ...
                        CyIntDisable(UART_USB_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                    UART_USB_txBufferWrite = loc_txBufferWrite;
                    #if ((UART_USB_TXBUFFERSIZE > UART_USB_MAX_BYTE_VALUE) && (CY_PSOC3))
                        CyIntEnable(UART_USB_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                }
    ...
    *******************************************************************************/
    void UART_USB_PutString(const char8 string[]) 
    {
        ...
        if(UART_USB_initVar != 0u)
        {
            ...
            {
                UART_USB_PutChar((uint8)string[buf_index]);
                buf_index++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if(UART_USB_initVar != 0u)
        {
            do
            {
                UART_USB_PutChar(string[buf_index]);
                buf_index++;
    ...
    *******************************************************************************/
    void UART_USB_LoadTxConfig(void) 
    {
        ...
        UART_USB_RXBITCTR_PERIOD_REG = UART_USB_HD_TXBITCTR_INIT;
            ...
            CY_GET_REG8(UART_USB_RXSTATUS_PTR);
        #endif /* CY_UDB_V0 */
    }
