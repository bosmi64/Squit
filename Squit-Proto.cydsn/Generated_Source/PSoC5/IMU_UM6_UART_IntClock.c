/*******************************************************************************
* File Name: IMU_UM6_UART_IntClock.c
* Version 2.0
*
*  Description:
...
/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_Start
********************************************************************************
*
* Summary:
...
void IMU_UM6_UART_IntClock_Start(void) 
{
    ...
    IMU_UM6_UART_IntClock_CLKEN |= IMU_UM6_UART_IntClock_CLKEN_MASK;
	IMU_UM6_UART_IntClock_CLKSTBY |= IMU_UM6_UART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void IMU_UM6_UART_IntClock_Stop(void) 
{
    ...
    IMU_UM6_UART_IntClock_CLKEN &= (uint8)(~IMU_UM6_UART_IntClock_CLKEN_MASK);
	IMU_UM6_UART_IntClock_CLKSTBY &= (uint8)(~IMU_UM6_UART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((IMU_UM6_UART_IntClock_CLKEN & IMU_UM6_UART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = IMU_UM6_UART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = IMU_UM6_UART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* IMU_UM6_UART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(IMU_UM6_UART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(IMU_UM6_UART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = IMU_UM6_UART_IntClock_GetSourceRegister();
    uint16 oldDivider = IMU_UM6_UART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = IMU_UM6_UART_IntClock_CLKEN & IMU_UM6_UART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(IMU_UM6_UART_IntClock_DIV_PTR, clkDivider);
                IMU_UM6_UART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                IMU_UM6_UART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(IMU_UM6_UART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((IMU_UM6_UART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(IMU_UM6_UART_IntClock_DIV_PTR, clkDivider);
				IMU_UM6_UART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(IMU_UM6_UART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    IMU_UM6_UART_IntClock_MOD_SRC |= modeBitMask & (uint8)IMU_UM6_UART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    IMU_UM6_UART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(IMU_UM6_UART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return IMU_UM6_UART_IntClock_MOD_SRC & (uint8)(IMU_UM6_UART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = IMU_UM6_UART_IntClock_GetDividerRegister();
    uint8 oldSrc = IMU_UM6_UART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        IMU_UM6_UART_IntClock_MOD_SRC |= CYCLK_SSS;
        IMU_UM6_UART_IntClock_MOD_SRC =
            (IMU_UM6_UART_IntClock_MOD_SRC & (uint8)(~IMU_UM6_UART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return IMU_UM6_UART_IntClock_MOD_SRC & IMU_UM6_UART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    IMU_UM6_UART_IntClock_PHASE = clkPhase & IMU_UM6_UART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: IMU_UM6_UART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return IMU_UM6_UART_IntClock_PHASE & IMU_UM6_UART_IntClock_PHASE_MASK;
}
