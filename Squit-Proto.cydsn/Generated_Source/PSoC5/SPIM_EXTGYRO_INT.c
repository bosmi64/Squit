...
/* `#START SPIM_EXTGYRO_ISR_START_DEF` */

/* `#END` */


/*******************************************************************************
* Function Name: SPIM_EXTGYRO_TX_ISR
********************************************************************************
*
* Summary:
...
CY_ISR(SPIM_EXTGYRO_TX_ISR)
{
    ...
    /* User code required at start of ISR */
    /* `#START SPIM_EXTGYRO_TX_ISR_START` */

    /* `#END` */

    #if(SPIM_EXTGYRO_TX_SOFTWARE_BUF_ENABLED)
        /* Check if TX data buffer is not empty and there is space in TX FIFO */
        while(SPIM_EXTGYRO_txBufferRead != SPIM_EXTGYRO_txBufferWrite)
        {
            tmpStatus = SPIM_EXTGYRO_GET_STATUS_TX(SPIM_EXTGYRO_swStatusTx);
            SPIM_EXTGYRO_swStatusTx = tmpStatus;

            if(0u != (SPIM_EXTGYRO_swStatusTx & SPIM_EXTGYRO_STS_TX_FIFO_NOT_FULL))
            {
                if(0u == SPIM_EXTGYRO_txBufferFull)
                {
                ...
                CY_SET_REG16(SPIM_EXTGYRO_TXDATA_PTR,
                    SPIM_EXTGYRO_txBuffer[SPIM_EXTGYRO_txBufferRead]);
            }
            else
            {
                break;
...
/*******************************************************************************
* Function Name: SPIM_EXTGYRO_RX_ISR
********************************************************************************
*
* Summary:
...
CY_ISR(SPIM_EXTGYRO_RX_ISR)
{
    ...
    /* User code required at start of ISR */
    /* `#START SPIM_EXTGYRO_RX_ISR_START` */
    ...
    /* `#END` */
        ...
        SPIM_EXTGYRO_swStatusRx = tmpStatus;

        /* Check if RX data FIFO has some data to be moved into the RX Buffer */
        while(0u != (SPIM_EXTGYRO_swStatusRx & SPIM_EXTGYRO_STS_RX_FIFO_NOT_EMPTY))
        {
            rxData = CY_GET_REG16(SPIM_EXTGYRO_RXDATA_PTR);

            /* Set next pointer. */
            SPIM_EXTGYRO_rxBufferWrite++;
            if(SPIM_EXTGYRO_rxBufferWrite >= SPIM_EXTGYRO_RX_BUFFER_SIZE)
            {
                SPIM_EXTGYRO_rxBufferWrite = 0u;
            }

            if(SPIM_EXTGYRO_rxBufferWrite == SPIM_EXTGYRO_rxBufferRead)
            {
                ...
                SPIM_EXTGYRO_rxBufferFull = 1u;
            }

            /* Move data from the FIFO to the Buffer */
            SPIM_EXTGYRO_rxBuffer[SPIM_EXTGYRO_rxBufferWrite] = rxData;
