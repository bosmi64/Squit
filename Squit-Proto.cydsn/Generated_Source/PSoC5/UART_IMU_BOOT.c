...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_IMU) || \
         ...
         
{
    ...
    do
    {
        UART_IMU_PutChar(pData[buf_index]);
        buf_index++;
         ...
         
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_IMU_GetRxBufferSize())
        {
            ...
            */
            do
            {
                ...
                oldDataCount = UART_IMU_GetRxBufferSize();
                CyDelay(UART_IMU_BYTE2BYTE_TIME_OUT);
            }while(UART_IMU_GetRxBufferSize() > oldDataCount);
    ...
    *count = 0u;
    ...
    while(UART_IMU_GetRxBufferSize() > 0u)
    {
        tempCount = UART_IMU_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_IMU_ReadRxData();
                dataIndexCntr++;
            }
