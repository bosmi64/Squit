...
/*******************************************************************************
* Function Name: Clock_UARTs_Start
********************************************************************************
*
* Summary:
...
void Clock_UARTs_Start(void) 
{
    ...
    Clock_UARTs_CLKEN |= Clock_UARTs_CLKEN_MASK;
	Clock_UARTs_CLKSTBY |= Clock_UARTs_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_UARTs_Stop
********************************************************************************
*
* Summary:
...
void Clock_UARTs_Stop(void) 
{
    ...
    Clock_UARTs_CLKEN &= (uint8)(~Clock_UARTs_CLKEN_MASK);
	Clock_UARTs_CLKSTBY &= (uint8)(~Clock_UARTs_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_UARTs_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_UARTs_CLKEN & Clock_UARTs_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_UARTs_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_UARTs_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_UARTs__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_UARTs_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_UARTs_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_UARTs_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_UARTs_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_UARTs_GetSourceRegister();
    uint16 oldDivider = Clock_UARTs_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_UARTs_CLKEN & Clock_UARTs_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_UARTs_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_UARTs_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_UARTs_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_UARTs_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_UARTs_MOD_SRC |= modeBitMask & (uint8)Clock_UARTs_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_UARTs_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_UARTs_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_UARTs_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_UARTs_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_UARTs_MOD_SRC & (uint8)(Clock_UARTs_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_UARTs_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_UARTs_GetDividerRegister();
    uint8 oldSrc = Clock_UARTs_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_UARTs_MOD_SRC |= CYCLK_SSS;
        Clock_UARTs_MOD_SRC =
            (Clock_UARTs_MOD_SRC & (uint8)(~Clock_UARTs_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_UARTs_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_UARTs_MOD_SRC & Clock_UARTs_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_UARTs_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_UARTs_PHASE = clkPhase & Clock_UARTs_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_UARTs_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_UARTs_PHASE & Clock_UARTs_PHASE_MASK;
}
