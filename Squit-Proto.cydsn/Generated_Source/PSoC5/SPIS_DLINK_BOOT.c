...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPIS_DLINK) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: SPIS_DLINK_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
void SPIS_DLINK_CyBtldrCommStart(void) CYSMALL 
{
    ...
    SPIS_DLINK_Start();
    SPIS_DLINK_ClearRxBuffer();
    SPIS_DLINK_ClearTxBuffer();
}


/*******************************************************************************
* Function Name: SPIS_DLINK_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPIS_DLINK_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPIS_DLINK_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    (void) SPIS_DLINK_ReadTxStatus();
    
    /* Write TX data using blocking function */
    #if(SPIS_DLINK_ModeUseZero == 1u)
        SPIS_DLINK_WriteTxDataZero(pData[buf_index]);
        buf_index++;
    ...
    do
    {
        SPIS_DLINK_WriteTxData(pData[buf_index]);
        buf_index++;
    }while(buf_index < size);

    while(0u != timeoutMs)
    {
        if(0u != (SPIS_DLINK_ReadTxStatus() & SPIS_DLINK_STS_SPI_DONE))
        {
            /* Return success code */
            *count = size;
...
/*******************************************************************************
* Function Name: SPIS_DLINK_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        * waiting for more data reception */
        if(0u != SPIS_DLINK_GetRxBufferSize()) 
        {
            ...
            * the program execution will break from the data awaiting loop with status=CYRET_SUCCESS */
            do
            {
                oldDataCount = SPIS_DLINK_GetRxBufferSize();
                CyDelay(SPIS_DLINK_BYTE2BYTE_TIME_OUT);
            }while(SPIS_DLINK_GetRxBufferSize() > oldDataCount);
            status = CYRET_SUCCESS; 
            break;
        }
    ...
    *count = 0u;
    ...
    while(SPIS_DLINK_GetRxBufferSize() > 0u)
    {
        tempCount = SPIS_DLINK_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = SPIS_DLINK_ReadRxData(); 
                dataIndexCntr++;
            }
...
#endif /* if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPIS_DLINK) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)) */
