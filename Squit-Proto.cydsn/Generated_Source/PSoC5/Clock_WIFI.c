...
/*******************************************************************************
* Function Name: Clock_WIFI_Start
********************************************************************************
*
* Summary:
...
void Clock_WIFI_Start(void) 
{
    ...
    Clock_WIFI_CLKEN |= Clock_WIFI_CLKEN_MASK;
	Clock_WIFI_CLKSTBY |= Clock_WIFI_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_WIFI_Stop
********************************************************************************
*
* Summary:
...
void Clock_WIFI_Stop(void) 
{
    ...
    Clock_WIFI_CLKEN &= (uint8)(~Clock_WIFI_CLKEN_MASK);
	Clock_WIFI_CLKSTBY &= (uint8)(~Clock_WIFI_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_WIFI_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_WIFI_CLKEN & Clock_WIFI_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_WIFI_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_WIFI_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_WIFI__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_WIFI_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_WIFI_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_WIFI_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_WIFI_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_WIFI_GetSourceRegister();
    uint16 oldDivider = Clock_WIFI_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_WIFI_CLKEN & Clock_WIFI_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_WIFI_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_WIFI_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_WIFI_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_WIFI_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_WIFI_MOD_SRC |= modeBitMask & (uint8)Clock_WIFI_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_WIFI_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_WIFI_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_WIFI_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_WIFI_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_WIFI_MOD_SRC & (uint8)(Clock_WIFI_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_WIFI_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_WIFI_GetDividerRegister();
    uint8 oldSrc = Clock_WIFI_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_WIFI_MOD_SRC |= CYCLK_SSS;
        Clock_WIFI_MOD_SRC =
            (Clock_WIFI_MOD_SRC & (uint8)(~Clock_WIFI_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_WIFI_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_WIFI_MOD_SRC & Clock_WIFI_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_WIFI_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_WIFI_PHASE = clkPhase & Clock_WIFI_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_WIFI_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_WIFI_PHASE & Clock_WIFI_PHASE_MASK;
}
