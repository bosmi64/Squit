...
static uint8 XDATA I2C_COULOMB_slReadBuf[I2C_COULOMB_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer: Host reads responses from it */
static uint8 XDATA I2C_COULOMB_slWriteBuf[I2C_COULOMB_BTLDR_SIZEOF_WRITE_BUFFER];


/*******************************************************************************
* Function Name: I2C_COULOMB_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_COULOMB_slWriteBuf - used to store received command.
*  I2C_COULOMB_slReadBuf - used to store response.
...
{
    /* Set Write buffer */
    I2C_COULOMB_SlaveInitWriteBuf(I2C_COULOMB_slWriteBuf, I2C_COULOMB_BTLDR_SIZEOF_WRITE_BUFFER);

    /* Set Read buffer which has zero elements */
    I2C_COULOMB_SlaveInitReadBuf(I2C_COULOMB_slReadBuf, 0u);
...
/*******************************************************************************
* Function Name: I2C_COULOMB_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_COULOMB_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
void I2C_COULOMB_CyBtldrCommReset(void) CYSMALL 
{
    ...
    I2C_COULOMB_slStatus = 0u;
}


/*******************************************************************************
* Function Name: I2C_COULOMB_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus I2C_COULOMB_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
/*******************************************************************************
* Function Name: I2C_COULOMB_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from the host */
        {
            ...
            if(0u != (I2C_COULOMB_slStatus & I2C_COULOMB_SSTAT_WR_CMPLT))
            {
                /* How many bytes the host has been written */
                byteCount = I2C_COULOMB_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2C_COULOMB_slWriteBuf,
                              I2C_COULOMB_MIN_UNT16(byteCount, size));
