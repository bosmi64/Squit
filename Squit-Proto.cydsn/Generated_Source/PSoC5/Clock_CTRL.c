...
/*******************************************************************************
* Function Name: Clock_CTRL_Start
********************************************************************************
*
* Summary:
...
void Clock_CTRL_Start(void) 
{
    ...
    Clock_CTRL_CLKEN |= Clock_CTRL_CLKEN_MASK;
	Clock_CTRL_CLKSTBY |= Clock_CTRL_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_CTRL_Stop
********************************************************************************
*
* Summary:
...
void Clock_CTRL_Stop(void) 
{
    ...
    Clock_CTRL_CLKEN &= (uint8)(~Clock_CTRL_CLKEN_MASK);
	Clock_CTRL_CLKSTBY &= (uint8)(~Clock_CTRL_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_CTRL_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_CTRL_CLKEN & Clock_CTRL_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_CTRL_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_CTRL_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_CTRL__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_CTRL_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_CTRL_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_CTRL_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_CTRL_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_CTRL_GetSourceRegister();
    uint16 oldDivider = Clock_CTRL_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_CTRL_CLKEN & Clock_CTRL_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_CTRL_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_CTRL_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_CTRL_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_CTRL_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_CTRL_MOD_SRC |= modeBitMask & (uint8)Clock_CTRL_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_CTRL_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_CTRL_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_CTRL_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_CTRL_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_CTRL_MOD_SRC & (uint8)(Clock_CTRL_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_CTRL_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_CTRL_GetDividerRegister();
    uint8 oldSrc = Clock_CTRL_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_CTRL_MOD_SRC |= CYCLK_SSS;
        Clock_CTRL_MOD_SRC =
            (Clock_CTRL_MOD_SRC & (uint8)(~Clock_CTRL_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_CTRL_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_CTRL_MOD_SRC & Clock_CTRL_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_CTRL_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_CTRL_PHASE = clkPhase & Clock_CTRL_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_CTRL_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_CTRL_PHASE & Clock_CTRL_PHASE_MASK;
}
