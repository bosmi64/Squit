...
I2C_COLOUMB_BACKUP_STRUCT I2C_COLOUMB_backup =
{
    I2C_COLOUMB_DISABLE, /* enableState */

    #if(I2C_COLOUMB_FF_IMPLEMENTED)
        I2C_COLOUMB_DEFAULT_XCFG,  /* xcfg */
        I2C_COLOUMB_DEFAULT_CFG,   /* cfg  */

        #if(I2C_COLOUMB_MODE_SLAVE_ENABLED)
            I2C_COLOUMB_DEFAULT_ADDR, /* addr */
        #endif /* (I2C_COLOUMB_MODE_SLAVE_ENABLED) */

        #if(CY_PSOC5A)
            LO8(I2C_COLOUMB_DEFAULT_DIVIDE_FACTOR),  /* div */
        #else
            LO8(I2C_COLOUMB_DEFAULT_DIVIDE_FACTOR), /* div1 */
            HI8(I2C_COLOUMB_DEFAULT_DIVIDE_FACTOR), /* div2 */
        #endif /* (CY_PSOC5A) */

    #else  /* (I2C_COLOUMB_UDB_IMPLEMENTED) */
        I2C_COLOUMB_DEFAULT_CFG,    /* control */

        #if(CY_UDB_V0)
            I2C_COLOUMB_INT_ENABLE_MASK, /* aux_ctl */

            #if(I2C_COLOUMB_MODE_SLAVE_ENABLED)
                I2C_COLOUMB_DEFAULT_ADDR, /* addr_d0 */
            #endif /* (I2C_COLOUMB_MODE_SLAVE_ENABLED) */
        #endif /* (CY_UDB_V0) */
    #endif /* (I2C_COLOUMB_FF_IMPLEMENTED) */

    #if(I2C_COLOUMB_TIMEOUT_ENABLED)
        I2C_COLOUMB_DEFAULT_TMOUT_PERIOD,
        I2C_COLOUMB_DEFAULT_TMOUT_INTR_MASK,

        #if(I2C_COLOUMB_TIMEOUT_PRESCALER_ENABLED && CY_UDB_V0)
            I2C_COLOUMB_DEFAULT_TMOUT_PRESCALER_PRD,
        #endif /* (I2C_COLOUMB_TIMEOUT_PRESCALER_ENABLED) */

    #endif /* (I2C_COLOUMB_TIMEOUT_ENABLED) */
};

#if((I2C_COLOUMB_FF_IMPLEMENTED) && (I2C_COLOUMB_WAKEUP_ENABLED))
    volatile uint8 I2C_COLOUMB_wakeupSource;
#endif /* ((I2C_COLOUMB_FF_IMPLEMENTED) && (I2C_COLOUMB_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: I2C_COLOUMB_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  I2C_COLOUMB_backup - used to save component configuration and
...
{
    #if(I2C_COLOUMB_FF_IMPLEMENTED)
        #if(I2C_COLOUMB_WAKEUP_ENABLED)
            uint8 enableInterrupts;
        #endif /* (I2C_COLOUMB_WAKEUP_ENABLED) */

        /* Store regiters in either Sleep mode */
        I2C_COLOUMB_backup.cfg  = I2C_COLOUMB_CFG_REG;
        I2C_COLOUMB_backup.xcfg = I2C_COLOUMB_XCFG_REG;
        ...
        #if(CY_PSOC5A)
            I2C_COLOUMB_backup.clkDiv   = I2C_COLOUMB_CLKDIV_REG;
        #else
            ...
            I2C_COLOUMB_CFG_REG &= ((uint8) ~I2C_COLOUMB_ENABLE_MASTER);

            /* Enable the I2C regulator backup */
            enableInterrupts = CyEnterCriticalSection();
            I2C_COLOUMB_PWRSYS_CR1_REG |= I2C_COLOUMB_PWRSYS_CR1_I2C_REG_BACKUP;
            CyExitCriticalSection(enableInterrupts);

            /* 1) Set force NACK to ignore I2C transactions
               2) Wait while I2C will be ready go to Sleep
               3) These bits are cleared on wake up */
            I2C_COLOUMB_XCFG_REG |= I2C_COLOUMB_XCFG_FORCE_NACK;
            while(0u == (I2C_COLOUMB_XCFG_REG & I2C_COLOUMB_XCFG_RDY_TO_SLEEP))
            {
            ...
            (void) CyIntSetVector(I2C_COLOUMB_ISR_NUMBER, &I2C_COLOUMB_WAKEUP_ISR);
            I2C_COLOUMB_wakeupSource = 0u;
        ...
        I2C_COLOUMB_backup.control = (I2C_COLOUMB_CFG_REG & I2C_COLOUMB_CTRL_ANY_ADDRESS_MASK);

        #if(CY_UDB_V0)
            /* Store interrupt mask bits */
            I2C_COLOUMB_backup.intMask = I2C_COLOUMB_INT_MASK_REG;

            #if(I2C_COLOUMB_MODE & I2C_COLOUMB_MODE_SLAVE)
                I2C_COLOUMB_backup.addr = I2C_COLOUMB_ADDR_REG;
            #endif /* (I2C_COLOUMB_MODE & I2C_COLOUMB_MODE_SLAVE) */
    ...
    #if(I2C_COLOUMB_TIMEOUT_ENABLED)
        I2C_COLOUMB_TimeoutSaveConfig();   /* Save Timeout config */
    #endif /* (I2C_COLOUMB_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_COLOUMB_Sleep
********************************************************************************
*
* Summary:
...
void I2C_COLOUMB_Sleep(void) 
{
        ...
        I2C_COLOUMB_backup.enableState = I2C_COLOUMB_DISABLE;
        ...
        I2C_COLOUMB_backup.enableState = ((uint8) I2C_COLOUMB_IS_ENABLED);

        if(I2C_COLOUMB_IS_ENABLED)
        {
            I2C_COLOUMB_Stop();
        }
    #endif /* (I2C_COLOUMB_WAKEUP_ENABLED) */

    I2C_COLOUMB_SaveConfig();
}


/*******************************************************************************
* Function Name: I2C_COLOUMB_RestoreConfig
********************************************************************************
*
* Summary:
...
void I2C_COLOUMB_RestoreConfig(void) 
{
        ...
        if(I2C_COLOUMB_CHECK_PWRSYS_I2C_BACKUP)    /* Enabled if was in Sleep */
        {
            /* Disable back-up regulator */
            enableInterrupts = CyEnterCriticalSection();
            I2C_COLOUMB_PWRSYS_CR1_REG &= ((uint8) ~I2C_COLOUMB_PWRSYS_CR1_I2C_REG_BACKUP);
            CyExitCriticalSection(enableInterrupts);

            /* Re-enable Master */
            I2C_COLOUMB_CFG_REG = I2C_COLOUMB_backup.cfg;
        }
        else /* The I2C_REG_BACKUP was cleaned by PM API: it means Hibernate or wake-up not set */
        {
                ...
                I2C_COLOUMB_backup.enableState = I2C_COLOUMB_ENABLE;
            #endif /* (I2C_COLOUMB_WAKEUP_ENABLED) */

            /* Restore component registers: Hibernate disable power */
            I2C_COLOUMB_XCFG_REG = I2C_COLOUMB_backup.xcfg;
            I2C_COLOUMB_CFG_REG  = I2C_COLOUMB_backup.cfg;
            ...
            #if(CY_PSOC5A)
                I2C_COLOUMB_CLKDIV_REG  = I2C_COLOUMB_backup.clkDiv;
            #else
            ...
            if(0u != I2C_COLOUMB_wakeupSource)
            {
                I2C_COLOUMB_SetPendingInt();   /* Generate interrupt to process incomming transcation */
            }
            I2C_COLOUMB_EnableInt();
        #endif /* (I2C_COLOUMB_WAKEUP_ENABLED) */
            ...
            I2C_COLOUMB_INT_MASK_REG |= I2C_COLOUMB_backup.intMask;

            enableInterrupts = CyEnterCriticalSection();
            I2C_COLOUMB_INT_ENABLE_REG |= I2C_COLOUMB_INT_ENABLE_MASK;
            CyExitCriticalSection(enableInterrupts);
        ...
        I2C_COLOUMB_CFG_REG = I2C_COLOUMB_backup.control;

    #endif /* (I2C_COLOUMB_FF_IMPLEMENTED) */

    #if(I2C_COLOUMB_TIMEOUT_ENABLED)
        I2C_COLOUMB_TimeoutRestoreConfig();
    #endif /* (I2C_COLOUMB_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_COLOUMB_Wakeup
********************************************************************************
*
* Summary:
*  Wakeup on address match enabled: enables I2C Master (if was enabled before go
...
{
    I2C_COLOUMB_RestoreConfig();   /* Restore I2C register settings */

    /* Restore component enable state */
    if(0u != I2C_COLOUMB_backup.enableState)
    {
        I2C_COLOUMB_Enable();
        I2C_COLOUMB_EnableInt();
    }
