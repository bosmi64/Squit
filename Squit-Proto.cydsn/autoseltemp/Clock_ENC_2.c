...
/*******************************************************************************
* Function Name: Clock_ENC_2_Start
********************************************************************************
* Summary:
...
void Clock_ENC_2_Start(void) 
{
    ...
    Clock_ENC_2_CLKEN |= Clock_ENC_2_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: Clock_ENC_2_Stop
********************************************************************************
* Summary:
...
void Clock_ENC_2_Stop(void) 
{
	...
	CYDEV_CHIP_REVISION_USED == CYDEV_CHIP_REVISION_5A_ES1))
/*******************************************************************************
* Function Name: Clock_ENC_2_StopBlock
********************************************************************************
* Summary:
...
{
    if (Clock_ENC_2_CLKEN & Clock_ENC_2_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = Clock_ENC_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_ENC_2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_ENC_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_ENC_2_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: Clock_ENC_2_StandbyPower
********************************************************************************
* Summary:
...
void Clock_ENC_2_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        Clock_ENC_2_CLKSTBY &= ~Clock_ENC_2_CLKSTBY_MASK;
    }
    else
    {
        Clock_ENC_2_CLKSTBY |= Clock_ENC_2_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: Clock_ENC_2_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_ENC_2_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = Clock_ENC_2_GetSourceRegister();
    uint16 oldDivider = Clock_ENC_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_ENC_2_CLKEN & Clock_ENC_2_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(Clock_ENC_2_DIV_PTR, clkDivider);
                Clock_ENC_2_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
/*******************************************************************************
* Function Name: Clock_ENC_2_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(Clock_ENC_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_ENC_2_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_ENC_2_SetModeRegister(uint8 clkMode) 
{
    Clock_ENC_2_MOD_SRC |= clkMode & Clock_ENC_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_ENC_2_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_ENC_2_ClearModeRegister(uint8 clkMode) 
{
    Clock_ENC_2_MOD_SRC &= ~clkMode | ~Clock_ENC_2_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_ENC_2_GetModeRegister
********************************************************************************
* Summary:
...
{
    return Clock_ENC_2_MOD_SRC & Clock_ENC_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_ENC_2_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = Clock_ENC_2_GetDividerRegister();
    uint8 oldSrc = Clock_ENC_2_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clock_ENC_2_MOD_SRC |= CYCLK_SSS;
        Clock_ENC_2_MOD_SRC =
            (Clock_ENC_2_MOD_SRC & ~Clock_ENC_2_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_ENC_2_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return Clock_ENC_2_MOD_SRC & Clock_ENC_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_ENC_2_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    Clock_ENC_2_PHASE = clkPhase & Clock_ENC_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_ENC_2_GetPhase
********************************************************************************
* Summary:
...
{
    return Clock_ENC_2_PHASE & Clock_ENC_2_PHASE_MASK;
}
